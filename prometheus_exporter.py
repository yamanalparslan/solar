...
solar_guc = Gauge('solar_power_watts', 'Inverter Anlik Uretim Gucu', ['slave_id'])
solar_voltaj = Gauge('solar_voltage_volts', 'Inverter DC Voltaj Degeri', ['slave_id'])
solar_akim = Gauge('solar_current_amps', 'Inverter DC Akim Degeri', ['slave_id'])
solar_sicaklik = Gauge('solar_temperature_celsius', 'Inverter İc Sicakligi', ['slave_id'])
solar_hata = Gauge('solar_error_status', 'Inverter Hata Kodu Durumu (107 ve 111)', ['slave_id', 'register'])
...
def update_metrics():
    ...
    """
    try:
        # Tuple: (slave_id, zaman, guc, voltaj, akim, sicaklik, hata_kodu, hata_kodu_111)
        cihaz_durumlari = veritabani.tum_cihazlarin_son_durumu()
        ...
        for cihaz in cihaz_durumlari:
            ...
            solar_guc.labels(slave_id=s_id).set(guc)
            solar_voltaj.labels(slave_id=s_id).set(voltaj)
            solar_akim.labels(slave_id=s_id).set(akim)
            solar_sicaklik.labels(slave_id=s_id).set(sicaklik)
            
            # Hata kodlarını ayrı labellarla kayıt et
            solar_hata.labels(slave_id=s_id, register="107").set(hk107)
            solar_hata.labels(slave_id=s_id, register="111").set(hk111)
